...
/**
 * Closure function generating SQL for a single PostgreSQL table.
 * @param table Target table definition.
 * @param ast Root Schema AST.
 * @returns Generated SQL CREATE TABLE statement.
 */
export const generatePostgresqlTable = (table: Table, ast: SchemaAST): string => {
    const lines: string[] = [];
    ...
    for (const column of table.columns) {
        ...
        if (column.enumId && ast.enums && ast.enums[column.enumId]) {
            pgType = `"${ast.enums[column.enumId].name}"`;
        }
        else if (column.constraints.isAutoIncrement) {
            pgType = column.type.toUpperCase() === "INTEGER" ? "SERIAL" : "BIGSERIAL";
        }
...
/**
 * Creates a PostgreSQL CodeGenerator function closure instance.
 * @returns CodeGenerator closure.
 */
export const createPostgresqlGenerator = (): CodeGenerator => {
    const generate = (ast: SchemaAST): string => {
        ...
        for (const table of sortedTables) {
            statements.push(generatePostgresqlTable(table, ast));
        }
