...
import {
  DataTypeRegistry,
  SQLiteDataTypeStrategy,
  PostgreSQLDataTypeStrategy,
  MySQLDataTypeStrategy
...
describe("DataTypeRegistry and Dialect Strategies", () => {
  ...
  it("should support registering a custom dialect strategy at runtime", () => {
    const customStrategy = {
      dialect: "oracle" as any,
      defaultType: "NUMBER",
      getDataTypes: () => [
        { type: "NUMBER", label: "NUMBER", category: "numeric" as const }
      ],
      getCategories: () => [
        { category: "numeric" as const, label: "Numeric", types: [{ type: "NUMBER", label: "NUMBER", category: "numeric" as const }] }
      ]
    };

    DataTypeRegistry.register(customStrategy);
    const resolved = DataTypeRegistry.getStrategy("oracle" as any);
    expect(resolved.defaultType).toBe("NUMBER");
  });
});
