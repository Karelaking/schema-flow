 ...
 * Remembers user consent preference in localStorage and provides a quick link to the full Cookie Policy page.
 */
export const CookieConsent: React.FC = (): React.ReactElement => {
    const [isVisible, setIsVisible] = useState<boolean>(false);

    useEffect(() => {
        const consent = localStorage.getItem("schema-flow-cookie-consent");
        if (!consent) {
            setIsVisible(true);
        }
    }, []);
    ...
    const handleAccept = (): void => {
        localStorage.setItem("schema-flow-cookie-consent", "accepted");
        setIsVisible(false);
    };

    const handleDecline = (): void => {
        localStorage.setItem("schema-flow-cookie-consent", "declined");
        setIsVisible(false);
    };
